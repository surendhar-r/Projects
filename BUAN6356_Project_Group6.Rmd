...
---
### Group Members: Betim Begzati, Charles Huff, Surendhar Radhakrishnan, Parisa Shahsavand, Neeraj Varakantam
...
#dropping columns that aren't described in the source
...
#huge chunk of data is/close to 0. small iqr
...
table(valid.df$Revenue, shop_logit2_pred[,2] > 0.5)
```
Running a model with repeated cross-validation produces the same result.
...
```
Best pruned tree with 4 splits, cp = .009085.
...
Random Forest generates the model with highest accuracy of 90.2% among other models.
